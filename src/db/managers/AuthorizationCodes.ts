...
class AuthorizationCodes {
  ...
  static revoke = async (authorizationCode: AuthorizationCode)
  : Promise<AuthorizationCode | undefined> => {

    try {
      return await getRepository(AuthorizationCode).remove(authorizationCode);
    } catch (error) {
      signale.error('[AuthorizationCodesOperationError :: revoke]');
      throw error;
    }
